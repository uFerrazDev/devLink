...
export function Admin(){
  ...
  return(
    <div className="flex items-center flex-col min-h-screen pb-7 px-2">
      ...
      {links.map(( itemCadastrado ) => (
        <article
        key={itemCadastrado.id}
        className="flex items-center justify-between w-11/12 max-w-xl rounded py-3 px-2 mb-2 select-none"
        style={{ backgroundColor: itemCadastrado.bg, color: itemCadastrado.color }}>
        <p>{itemCadastrado.name}</p>
        <div>
          <button className="border border-dashed p-1 rounded bg-neutral-900"
          onClick={ () => handleDeleteLink(itemCadastrado.id)}
          >
